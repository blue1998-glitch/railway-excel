...
with st.sidebar:
    ...
    if active_api_key:
        ...
        st.warning("⚠️ 請在此輸入或於 secrets 設定 API Key")
...
def load_excel_preserving_all(file_bytes, filename):
    ...
    if filename.lower().endswith(".xls"):
        try:
            ...
            raise ImportError("請在 requirements.txt 中加入 xlrd 套件。")
...
if st.button("🚀 開始智慧辨識與自動填表", type="primary", use_container_width=True):
    ...
    2. 擷取【車站名稱】（例如：豐富、苗栗、銅鑼、三義、泰安、后里、豐原、栗林、潭子、頭家厝、松竹、太原、精武、台中、五權、大慶、新烏日、烏日、成功、彰化、花壇、大村、員林、社頭、田中、二水等，不需包含'站'字）與進款【日期】（僅需日/號數，1-31 的整數）。
    3. 專注看左側【應解款數】大項目區塊，精確擷取各數值（若無填 0）：
       ...
       - 條碼支付退款(+) (填正數)
       - 應解總計 (報表上的應解總計數值)
       - 其他項目加總（若有存付運費、託收支票、補繳金額、繳回週轉金、其他短欠等款項，請計算其淨額填入其他；若無則填 0）
